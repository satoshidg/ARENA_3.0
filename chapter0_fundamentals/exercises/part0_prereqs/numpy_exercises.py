...
print("#4 bytes:", null_vector.nbytes)
...
print("#8:", reversed_arr)

#### 9. Create a 3x3 matrix with values ranging from 0 to 8 (★☆☆)
array = np.array([np.arange(0, 3), np.arange(3, 6), np.arange(6, 9)])
print("#9:", array)
...
arr = np.pad(arr, mode="constant", pad_width=1, constant_values=0)
print("#16:", arr)
...
'''
Notes:
    np.nan implements IEEE-754 a standard that defines how floating-point numbers are represetnted and how they behave.

'''
print(0 * np.nan) # np.nan: all arithmetic ops will results in np.nan
print(np.nan == np.nan) # False: np.nan does not equal to anything, even itself
print(np.inf > np.nan) # False: all comparisons with np.nan are always False
print(np.nan - np.nan) # np.nan: again, all arithmetic ops will results in np.nan
print(np.nan in set([np.nan])) # True: hashes of np.nan is constant
print(0.3 == 3 * 0.1) # False: floating-point rounding

#### 18. Create a 5x5 matrix with values 1,2,3,4 just below the diagonal (★☆☆)
arr = np.diag(np.arange(1, 5), -1).astype(float)
arr[arr == 0] = np.nan
print("#18:", arr)
...
color = np.dtype((np.ubyte,4))
arr = np.zeros(4, dtype=color)
print("#23:", arr, arr.dtype)
